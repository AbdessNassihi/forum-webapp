...
const NavigationItem = ({ icon, label, path, user, isSidebar, subItems = [] }) => {
    ...
    const handleClick = () => {
        if (subItems.length > 0) {
            ...
            if (user || path === '/') {
                ...
                navigate('/login', { state: { showToast: true, message: 'Log to your account.', toastType: 'info' } });
            }
        }
    };
    ...
    return (
        <div style={{ position: 'relative' }}>
            <div onClick={handleClick}>
                {isSidebar ? (<CDBSidebarMenuItem icon={icon} className="sidebar-item">{label}</CDBSidebarMenuItem>) : (<Nav.Item><Nav.Link><i className={icon}></i></Nav.Link></Nav.Item>)}
            </div>
            {isOpen && !isSidebar && (
