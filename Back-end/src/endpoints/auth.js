...
router.post('/logout', (req, res) => {
    ...
    req.logout((error) => {
        ...
        res.status(200).json({ code: 200, status: 'OK', message: 'User is Logged Out' });
    })
...
router.post('/register', validateAllReq, handleImageUpload, async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ code: 201, status: 'Created', message: 'User registered successfully', data: result });
    } catch (error) {
        if (error.code === 'ER_DUP_ENTRY') {
            ...
            return res.status(500).json({ error: { code: 500, status: 'Internal Server Error', message: 'Error while registering user', log: error.message } });
        }
    }
});
